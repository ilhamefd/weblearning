...
class Dosen_profil extends CI_Controller {
	...
	public function chg_profil($dsn_id){
		...
		$data=array('title'=>'Home',
					'isi'  =>'dosenpages/profil/chg_profil'
						);
		$data['login']	= $this->m_global->get_data_all('user', null, ['level'=>$status]);
	...
	public function chg_foto($dsn_id){
		$data=array('title'=>'Ganti Foto',
					'isi'  =>'dosenpages/profil/chg_foto'
						);
		$username = $this->session->userdata('username');
		$data['profil']=$this->m_global->get_data_all('dosen',null, ['kd_dosen'=>$username]);
	...
	public function act_chg_foto($dsn_id){
        ...
        if($_FILES['img']['name'])
        {
            if ($this->upload->do_upload('img'))
            {
                ...
                redirect('dosen_profil'); //jika berhasil maka akan ditampilkan view vupload
            }else{
                ...
                redirect('dosen_profil'); //jika gagal maka akan ditampilkan form upload
            }
        }
	}
}